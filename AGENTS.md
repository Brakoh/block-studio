...
src/scripts/overseers-cctv.js    — modello 3D CCTV nella Cornice (Three.js, FBX)
...
assets/3d/                       — modello FBX CCTV per OVERSEERS
...
- **Cornice**: i quattro poli sono pollice e indice di ciascuna mano. Quando le quattro dita sono abbastanza distanti, la maschera compare in fade in (nero a opacità 75%, outline 1px verde); si chiude in fade out se le dita si avvicinano o la Cornice sparisce. Il modello 3D `assets/3d/cctv.fbx` resta ancorato al volto (non segue la maschera), lente nella stessa direzione della faccia. Dal basso pende un tubo, da sopra tre cavi agganciati al retro; dondolano con la testa. Fuori dalla maschera Feed e Blob restano com’è.
...
- **OVERSEERS**: `studio-ui.js` per chrome/linea; `overseers-scene.js` per canvas 2D (webcam, pixel, Blob) + MediaPipe Pose/Face/Hands (CDN Tasks Vision); `overseers-cctv.js` per il modello 3D nella Cornice (Three.js, FBXLoader). Nessun audio.
...
| Cornice con pollice e indice distanti (OVERSEERS) | Maschera in fade in (nero, outline verde); fade out se le dita si chiudono. CCTV ancorata al volto |
